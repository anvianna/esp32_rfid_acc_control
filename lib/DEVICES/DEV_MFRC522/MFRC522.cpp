...
{
	if (spi_handle == nullptr || spi_handle == nullptr)
		return;
...
)
{
	...
	// SELENE transfer
	byte tx_data[2] = {reg, value}; // Sending register address followed by valued to write
...
)
{
	...
	// Selene Transfer
	byte total[count + 1];
	total[0] = reg; // Storing Register Address
	// Storing values to writing in the FIFO
...
)
{
	...
	// esp_timer is in microseconds
	const int64_t deadline = esp_timer_get_time() + 89000;
	...
	do
	{
		...
		// Supressed  @todo verify if it is important
		taskYIELD(); // yield();
	}
...
void MFRC522::PCD_Init()
{
	...
	/* // Supressed because DrvSPI already initialize the ChipSelect
...
void MFRC522::PCD_SoftPowerUp()
{
	...
	const int64_t timeout = esp_timer_get_time() + 500000; // create timer for timeout (just in case)
...
)
{
	...
	// esp_timer is in microseconds
	const int64_t deadline = esp_timer_get_time() + 36000;
...
void MFRC522::PCD_DumpVersionToSerial()
{
	...
	case 0x88:
		printf(" = (clone)\n\r");
		break; // Serial.println(F(" = (clone)"));  break;
	case 0x90:
		printf(" = v0.0\n\r");
		break; // Serial.println(F(" = v0.0"));     break;
	case 0x91:
		printf(" = v1.0\n\r");
		break; // Serial.println(F(" = v1.0"));     break;
	case 0x92:
		printf(" = v2.0\n\r");
		break; // Serial.println(F(" = v2.0"));     break;
	case 0x12:
		printf(" = counterfeit chip\n\r");
		break; // Serial.println(F(" = counterfeit chip"));     break;
	default:
